...
use crate::component_system::command_parser::{self as cmd};
...
impl Component for Misc {
    ...
    async fn command(&self, _: &FrameworkConfig, ctx: &Context, msg: &Message) -> CommandMatch {
        ...
        match matched.get_command() {
            "ping" => match utils::has_permission(ctx, msg, matched.permission).await {
                Ok(true) => Self::send_message(ctx, msg, "Pong!").await,
                Ok(false) => match utils::send::no_perm(ctx, msg.channel_id).await {
...
impl Misc {
    pub fn new(app_id: ApplicationId, bot_permissions: u64) -> Misc {
        Misc {
            node: cmd::Node::new().add_command(
                cmd::Command::new("ping").set_help("Permet d'avoir une réponse du bot"),
            ),
            app_id,
            bot_permissions,
        }
    ...
    async fn on_applications_command(
        ...
        if app_cmd.get_guild_id().is_none() {
