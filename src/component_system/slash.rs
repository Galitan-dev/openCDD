...
pub fn register_root_with_perm(
    node: &cmd::Node,
    permission: bool,
) -> Vec<CreateApplicationCommand> {
    let group_iter = node.groups.list().map(|group| {
        ...
        app_cmd.set_options(
            group
                ...
                .list()
                .map(register_group)
                .chain(group.commands().list().map(register_command))
                .collect(),
        );
        app_cmd
    });
    let command_iter = node.commands.list().map(|command| {
        let mut app_cmd = CreateApplicationCommand::default();
        app_cmd.name(command.name());
        app_cmd.default_permission(permission);
        command.help().map(|help| app_cmd.description(help));
        app_cmd.set_options(command.params().iter().map(register_argument).collect());
        app_cmd
    });
